...
BEGIN
    ...
    -- 步骤1：先更新所有Run的任务引用（在删除或更新任务之前）
    -- 将旧任务ID改为新任务ID，避免外键约束错误
    ...
    RAISE NOTICE '✓ 已更新Run的任务引用';

    -- 步骤2：检查新任务是否已存在
    IF EXISTS (SELECT 1 FROM world_quests WHERE id = quest_id_var) THEN
        RAISE NOTICE '⚠ 任务已存在，将更新现有任务';

        -- 更新现有任务
        UPDATE world_quests
        SET
            quest_name = '森林深处的呼唤',
            description = '马库斯的表弟在暗影森林深处失踪已有数日。他最后的信件提到发现了一处古老遗迹，但此后便杳无音信。',
            ...
            quest_giver_npc_id = marcus_npc_id
        WHERE id = quest_id_var;

        RAISE NOTICE '✓ 已更新任务"森林深处的呼唤"';
    ELSE
        -- 创建新任务：与网格系统完全匹配

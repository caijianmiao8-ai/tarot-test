-- ========================================
-- Phase 1: 网格地图系统迁移（修复版）
-- ========================================
-- 先创建必要的地点和NPC，然后创建网格系统
...
INSERT INTO world_npcs (
    ...
    mood, mood_icon, secrets, is_alive, interaction_count
)
SELECT
    'npc_marcus_001',
    w.id,
    ...
    '马库斯',
    '商队主人',
    '精明而谨慎的商人，对暗影之森的异常情况很担忧',
    'worried',
    '😟',
    '他的商队最近在暗影之森边缘失踪，他怀疑不是普通的盗匪所为',
    true,
    0
FROM adventure_worlds w
WHERE w.world_name = '边境之地'
...
SELECT
    ...
    '老板娘艾琳',
    '酒馆老板',
    '热情健谈，消息灵通，对镇上的八卦了如指掌',
    'friendly',
    '😊',
    '她知道很多关于暗影之森的传说和近期失踪事件',
    true,
    0
FROM adventure_worlds w
WHERE w.world_name = '边境之地'
...
-- 获取 quest_shadow_forest_001 的 ID 并更新
DO $$
DECLARE
...
BEGIN
    ...
    RAISE NOTICE '✓ 已创建/验证必要的地点和NPC';
    RAISE NOTICE '✓ 已更新任务检查点添加 grid_id';
    RAISE NOTICE '';
    RAISE NOTICE '下一步：';
    RAISE NOTICE '1. 刷新应用页面';
    RAISE NOTICE '2. 进入游戏测试网格移动';
    RAISE NOTICE '3. 测试检查点完成';
    RAISE NOTICE '';
END $$;

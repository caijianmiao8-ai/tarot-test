...
⚠️ **任务剧情约束**
❌ 禁止编造任务描述之外的背景故事
❌ 禁止提及不在任务描述中的组织、事件、线索
❌ NPC对话必须围绕【当前目标】展开，不要编造额外剧情
✓ 只描述任务描述中明确提到的内容
✓ 引导玩家完成【当前目标】"""
            else:
                quest_info = f"""
...
3. 禁止让NPC主动说话（NPC只能在玩家用"与XX对话"、"问XX"、"找XX说话"时才能回应）
...
4. 玩家的行动是否包含"与XX对话"、"问XX"？如果没有，NPC不能说话！
5. 如果玩家问"我在哪"，我是否只说了位置名称而没有编造细节？
